...
import { selfUpdateUser as selfUpdateUserApi, updateUser as updateUserApi } from "../Api/apiUser.tsx";
...
function useUpdateUser() {
  ...
  const { mutate: updateUser, isPending: isUserUpdating, error: userUpdateError} = useMutation({
    mutationFn: async ({ userId, updatedData }: { userId: string; updatedData: Partial<UserUpdate> }) =>
      await updateUserApi(userId, updatedData),
    onSuccess: async () => queryClient.invalidateQueries({ queryKey: ["users"] }),
    onError: async (error) => toast.error(error?.data?.message || "Something went wrong"),
  });
...
function useUpdateAuthorizedUser() {
  ...
  const { mutate: updateUser, isPending: isUserUpdating, error: userUpdateError} =  useMutation({
    mutationFn: async (userUpdate: Partial<UserUpdate>) => await selfUpdateUserApi(userUpdate),
