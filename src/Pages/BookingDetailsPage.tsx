  ...
  if (!booking || !room) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <h2 className="text-xl font-bold text-red-600">Дані бронювання відсутні</h2>
        <p className="mt-2 text-gray-600">Схоже, ви перейшли сюди безпосередньо. Поверніться до списку бронювань.</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <Column className="container mx-auto px-4 py-8 w-[70%] mt-36">
        <h1 className="text-sm  mb-6 text-green-500">Your booking is confirmed</h1>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div>
              <h2 className="font-bold text-lg mb-2">Your {room?.type} booking is confirmed</h2>
              <Row className="align-center gap-4"><CheckIcon/>You are all set. The confirmation is sent to {email}.</Row>
            </div>
            ...
            <Column className="shadow p-4">
              ...
              <p>Protect your security by never sharing your
                personal or credit card information over the phone, by email or chat.</p>
            </Column>
            <Row className="justify-between">
              <Row className="gap-4 justify-center align-center items-center">
                ...
                <Column className="border-r pr-4">
                  <p>Check-in</p>
                  <p className="font-bold text-lg">{formatDate(booking.start_date)}</p>
                </Column>
               <Column>
                  <p>Check-out</p>
                  <p className="font-bold text-lg">{formatDate(booking.end_date)}</p>
                </Column>
              </Row>
              <img alt="Room image" src={room_image} className="w-36 h-28 rounded"/>
            </Row>
            ...
            <Row className="gap-4">
              ...
              <Column>
                ...
                <p>Capacity: {room.capacity}, Beds: {room.beds}, Area: {room.area}, Floor: {room.floor}</p>
              </Column>
            </Row>

            <div>
              <h3 className="font-semibold">Address</h3>
              <button className="text-blue-600 hover:underline">4759 Smith Road, North Fairfield, Ohio, USA</button>
            </div>
          </div>
