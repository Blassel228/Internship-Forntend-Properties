...
const ReviewsPanel = ({ handleSetSelected, isSelected, allReviewsCount, writeReviewCount, propertyReviewsCount }) => {
  const { goTo } = useNavigation();
  const { name, surname, image_data, username }= useSelector( (state: RootState) => state.authorizedUser.authorizedUser);

  const bgColor = stringToColor(username);
  const initial = username.charAt(0).toUpperCase();

  const image = image_data ? `data:image/jpeg;base64,${image_data}`
    : undefined;
    ...
    <Column className="shadow pt-4 w-96">
      <Row className="gap-4 pl-4 pb-4">
        ...
        <Column className="pt-4 pr-4">
          ...
          <p onClick={() => goTo(routers.personalData)} className="text-blue-600 cursor-pointer">Edit Profile</p>
        </Column>
      </Row>
      <Column>
        <SidebarMenuItem
